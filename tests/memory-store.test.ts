...
describe("createMemoryStore", () => {
  ...
  describe("sanitizeId (path traversal prevention)", () => {
    it("rejects id with directory traversal sequences", async () => {
      const dir = createTempDir();
      const store = await createMemoryStore({ memoryDir: dir });

      // get with traversal id should not access files outside memoryDir
      const result = await store.get("../../../etc/passwd");
      // Should either return null (basename strips path) or throw
      expect(result).toBeNull();
    });

    it("rejects id with forward slash path", async () => {
      const dir = createTempDir();
      const store = await createMemoryStore({ memoryDir: dir });

      const result = await store.get("foo/bar/baz");
      expect(result).toBeNull();
    });

    it("rejects empty id after sanitization", async () => {
      const dir = createTempDir();
      const store = await createMemoryStore({ memoryDir: dir });

      await expect(store.get("..")).rejects.toThrow("Invalid memory id");
    });

    it("delete with traversal id does not affect external files", async () => {
      const dir = createTempDir();
      const store = await createMemoryStore({ memoryDir: dir });

      const deleted = await store.delete("../../../etc/passwd");
      expect(deleted).toBe(false);
    });

    it("normal ids with hyphens and underscores work fine", async () => {
      const dir = createTempDir();
      const store = await createMemoryStore({ memoryDir: dir });

      const entry = await store.write("test content", "profile");
      // id generated by generateMemoryId should work
      const retrieved = await store.get(entry.id);
      expect(retrieved).not.toBeNull();
